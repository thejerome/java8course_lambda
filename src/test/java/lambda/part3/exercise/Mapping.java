...
public class Mapping {
    ...
    public static List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jobHistory) {
        return new MapHelper<>(jobHistory)
                .map(j -> j.withDuration(j.getDuration() + 1))
                .getList();
    }

    private static List<JobHistoryEntry> toUperCaseQA(List<JobHistoryEntry> jobHistory) {
        return new MapHelper<>(jobHistory)
                .map(j -> j.getPosition().equals("qa")? j.withPosition("QA") : j)
                .getList();
    }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        ...
                        .map(e -> e.withJobHistory(toUperCaseQA(e.getJobHistory())))
                        .getList();
