...
public class Mapping {
    ...
    private List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jh){
        return new MapHelper<JobHistoryEntry>(jh)
                .map( p -> p.withDuration(p.getDuration() + 1))
                .getList();
    }

    private List<JobHistoryEntry> upperForQA(List<JobHistoryEntry> jh){
        return new MapHelper<JobHistoryEntry>(jh)
                .map( p -> p.getPosition().equals("qa") ? p.withPosition("QA"): p)
                .getList();
    }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                        ...
                        .map(e -> e.withJobHistory(upperForQA(e.getJobHistory())))
                .getList();
    ...
    private static class LazyMapHelper<T, R> {
        ...
        public List<R> force() {
            ...
            list.forEach( p -> result.add(function.apply(p)));
            return result;
        }
