...
public class Mapping {
        ...
        final List<Employee> mappedEmployees =
                new MapHelper<>(employees)
                    ...
                    .map(e -> e.withJobHistory(
                        new MapHelper<>(e.getJobHistory())
                            .map(entry -> entry.withDuration(entry.getDuration() + 1))
                            .getList()))
                    .map(e -> e.withJobHistory(
                        new MapHelper<>(e.getJobHistory())
                            .map(entry -> entry.getPosition().equals("qa")
                                ? entry.withPosition("QA")
                                : entry)
                        .getList()
                    ))
                .getList();
    ...
    private static class LazyFlatMapHelper<T, R> {
        ...
        public static <T> LazyFlatMapHelper<T, T> from(List<T> list) {
            return new LazyFlatMapHelper<T, T>(list, t -> Lists.newArrayList());
        }
        ...
        public <R2> LazyFlatMapHelper<T, R2> flatMap(Function<R, List<R2>> f) {
            return new LazyFlatMapHelper<>(list, (T t) -> new MapHelper<>(function.apply(t))
                .flatMap(f).getList());
        }
    }
    ...
    public void lazy_mapping() {
        ...
        final List<Employee> mappedEmployees =
                LazyMapHelper.from(employees)
                    .map(e -> e.withPerson(e.getPerson().withFirstName("John")))
                    .map(e -> e.withJobHistory(
                        LazyMapHelper.from(e.getJobHistory())
                            .map(entry -> entry.withDuration(entry.getDuration() + 1))
                            .force()))
                    .map(e -> e.withJobHistory(
                        LazyMapHelper.from(e.getJobHistory())
