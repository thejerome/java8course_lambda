...
public class Mapping {
    ...
    private static List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> f) {
        return new MapHelper<JobHistoryEntry>(f)
            .map(e -> e.withDuration(e.getDuration() + 1))
            .getList();
    }

    private static List<JobHistoryEntry> replaceQA(List<JobHistoryEntry> f) {
        return new MapHelper<JobHistoryEntry>(f)
            .map(e -> e.getPosition()
                       .equals("qa") ? e.withPosition("QA") : e.withPosition(e.getPosition()))
            .getList();
    }
    ...
    private static class LazyMapHelper<T, R> {
        ...
        private <R2> Function<T, R2> combine(Function<T, R> f1, Function<R, R2> f2){
            return e -> f2.apply(f1.apply(e));
        }
