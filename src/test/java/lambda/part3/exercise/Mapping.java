...
public class Mapping {
    ...
    public List<JobHistoryEntry> addOneYear(List<JobHistoryEntry> jh) {
        return new MapHelper<JobHistoryEntry>(jh).map((j) -> j.withDuration(j
                .getDuration() + 1)).getList();
    }

    public List<JobHistoryEntry> renameToUpperCase(List<JobHistoryEntry> jh) {
        return new MapHelper<JobHistoryEntry>(jh).map(j -> j.getPosition().equals("qa")
                ? j.withPosition("QA") : j).getList();
    }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees = new MapHelper<>(employees)
                ...
                // replace qa with QA
                .map(e -> e.withJobHistory(renameToUpperCase(e.getJobHistory()))).getList();
    ...
    public void lazy_mapping() {
        ...
        final List<Employee> mappedEmployees =
                LazyMapHelper.from(employees)
                        ...
                        // replace qa with QA
                        .map(e -> e.withJobHistory(renameToUpperCase(e.getJobHistory())))
                        .force();
