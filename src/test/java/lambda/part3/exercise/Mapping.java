...
public class Mapping {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> f) {
            ...
            list.forEach(t -> arrayList.add(f.apply(t)));
            return new MapHelper<>(arrayList);
        }
    ...
    public void mapping() {
        ...
        final List<Employee> mappedEmployees =
            new MapHelper<>(employees)
                ...
                .map(e -> e.withJobHistory(changePosition(e)))
                .getList();
    ...
    private List<JobHistoryEntry> changePosition(Employee e) {
        return new MapHelper<>(e.getJobHistory())
            .map(j -> j.getPosition().equals("qa") ? j.withPosition("QA") : j).getList();

    }

    private List<JobHistoryEntry> addOneYear(Employee e) {
        return new MapHelper<>(e.getJobHistory())
            .map(j -> j.withDuration(j.getDuration() + 1)).getList();
    }
        ...
        final List<Employee> mappedEmployees =
            LazyMapHelper.from(employees)
                ...
                * */
                ...
                .map(employee -> employee.withJobHistory(addOneYear(employee)))
                .map(employee -> employee.withJobHistory(changePosition(employee)))
                .force();
